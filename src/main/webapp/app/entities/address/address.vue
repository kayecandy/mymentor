<template>
    <div>
        <h2 id="page-heading">
            <span v-text="$t('mymentorApp.address.home.title')" id="address-heading">Addresses</span>
            <router-link :to="{name: 'AddressCreate'}" tag="button" id="jh-create-entity" class="btn btn-primary float-right jh-create-entity create-address">
                ...
                <span  v-text="$t('mymentorApp.address.home.createLabel')">
        ...
        <div class="alert alert-warning" v-if="!isFetching && addresses && addresses.length === 0">
            <span v-text="$t('mymentorApp.address.home.notFound')">No addresses found</span>
        </div>
        <div class="table-responsive" v-if="addresses && addresses.length > 0">
            <table class="table table-striped">
                ...
                <tr>
                    ...
                    <th><span v-text="$t('mymentorApp.address.street')">Street</span></th>
                    <th><span v-text="$t('mymentorApp.address.streetNumber')">Street Number</span></th>
                    <th><span v-text="$t('mymentorApp.address.secondLine')">Second Line</span></th>
                    <th><span v-text="$t('mymentorApp.address.postcode')">Postcode</span></th>
                    <th><span v-text="$t('mymentorApp.address.city')">City</span></th>
                    <th></th>
                </tr>
                </thead>
                <tbody>
        ...
        <b-modal ref="removeEntity" id="removeEntity" >
            <span slot="modal-title"><span id="mymentorApp.address.delete.question" v-text="$t('entity.delete.title')">Confirm delete operation</span></span>
            <div class="modal-body">
                <p id="jhi-delete-address-heading" v-bind:title="$t('mymentorApp.address.delete.question')">Are you sure you want to delete this Address?</p>
            </div>
